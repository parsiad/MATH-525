...
and hence
\[
\rho(B)\leq\left\Vert B\right\Vert _{\infty}\leq\max_{i}\left\{ B_{ii}+\sum_{j\neq i}B_{ij}\right\} \leq\max_{i}\left\{ B_{ii}+s-B_{ii}\right\} =s.
\]
...
Then, by the same argument involving the Perron-Frobenius theorem
in the proof of Proposition \ref{prop:m_matrix}, $\rho(B)<s$ (if
$\rho(B)=s$, then $s$ is an eigenvalue of $B$, contradicting nonsingularity).
\end{proof}
\begin{cor}
