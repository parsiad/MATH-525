...
but it's not the most useful in probability.

Why? Consider two random variables $X$ and $Y$ which are equal to
a.s., but we can find $\omega$ such that $X(\omega)\neq Y(\omega)$.
Then, the alternating sequence $X,Y,X,Y,\ldots$ does not have a pointwise
limit, even though the two random variables are ``essentially''
the same!
...
\liminf_{n}\Lambda_{n} & =\left\{ \omega\mid\exists N\colon\forall n\geq N\colon\omega\notin\Lambda_{n}\right\} .
\end{align*}
\end{defn}
...
\begin{prop}
Let $(X_{n})_{n}$ be a sequence of ERV random variables and suppose
$X_{n}\rightarrow X$ in probability. Then, there exists a subsequence
$(n_{k})_{k}$ such that $X_{n_{k}}\rightarrow X$ a.e.
\end{prop}
\begin{proof}
...
\[
\mathbb{P}\left(\liminf_{m}\Lambda_{m}\right)=1.
\]

Recall now that
\[
\liminf_{m}\Lambda_{m}=\left\{ \omega\mid\exists N\colon\forall m\geq N\colon\omega\notin\Lambda_{n}\right\} .
\]
...
in probability (AU is a.u. convergence, but we haven't covered this
type of convergence). A solid line from one mode of convergence to
another indicates implication (e.g., a.e. convergence implies convergence
in probability). A dashed line means that we can extract a subsequence,
...
\par\end{centering}
\caption{Modes of convergence (John Cook)}
\end{figure}
