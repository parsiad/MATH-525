...
$f_{XY}$. If $f_{X}\neq0$, we can define the \emph{conditional density}
of $Y$ given $X=x$ by
\[
f_{Y\mid X}(y\mid x)={\displaystyle \frac{f_{XY}(x,v)}{f_{X}(x)}}.
\]
...
\end{lstlisting}
With $N$ equal to 10 billion, a run of the code above yields $\pi_{\text{approximate}}=3.142106$.
\end{example}
Even though we used 10 billion samples in the above, our approximation
...
\[
\pi_{\text{approximate}}-\pi=4\frac{S_{N}}{N}-\pi=O\left(\frac{1}{\sqrt{N}}\right)
\]
